    ...
    container.innerHTML = data.map(producer => `
      <div class="settings-list-item">
        <div class="settings-list-item-info">
          <div class="settings-list-item-name">${producer.name}</div>
        ...
        .insert([
          {
            ...
            name: name,
            provider_type: type,
            api_key_encrypted: apiKey, // Gerçek uygulamada şifrelenmeli
            api_secret_encrypted: apiSecret, // Gerçek uygulamada şifrelenmeli
            base_url: baseUrl,
            is_active: true
          }
        ])
...
window.testProducer = async (producerId) => {
  try {
    ...
    // Burada API bağlantı testi yapılacak
    setTimeout(() => {
